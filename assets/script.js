var apiKey = "APPID=b05a4fc379d4615e15c701c87e417a11";
...
$(document).ready(function () {
	...
	function findme() {
		...
		function error() {
			...
			$(".yourlocation").append(
				$("<p>").html(
					"<i>Sorry!! Unable to retrieve your location</i>"
				)
			);
		}
	}
	...
	if (localStorage["cities"]) {
		...
		cities.forEach((element) => {
			...
			liTag.addClass(
				"list-group-item list-group-item-action list-group-item-primary citybutton"
			);
		...
		$.ajax({
			...
			if (icon == "01d") {
				h3.append(
					$("<span>").append(
						$("<img>").attr({
							src: "assets/images/external-content.duckduckgo.png",
							height: "15%",
							width: "20%",
						})
					)
			...
			$(".currentcity").append(
				$("<p>").html(
					"<b>Humidity : " + response.main.humidity + "%</b>"
				)
			);
			$(".currentcity").append(
				$("<p>").html(
					"<b>Wind Speed : " + response.wind.speed + " MPH</b>"
				)
			);
	...
	function iamhere(latitude, longitude) {
		...
		$.ajax({
			...
			$(".yourlocation").append(
				$("<h4>").append(
					$("<span>").text(Math.round(response.main.temp) + " \u2109")
				)
			);
		});
	}
	...
	function getuvindex(lat, lon) {
		queryURL =
			baseURL + "uvi?" + apiKey + "&lat=" + lat + "&lon=" + lon;
